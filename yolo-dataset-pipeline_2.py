...
class YOLOPipeline:
    ...
    def parse_arguments(self):
        ...
        parser.add_argument("--data-source", type=str, choices=['roboflow', 'local'], default='roboflow',
                          help="Источник данных: roboflow или local (локальный)")
        ...
        # Параметры локального датасета
        parser.add_argument("--dataset-dir", type=str, default="dataset", 
                           help="Директория датасета (для загрузки или путь к локальному датасету)")
        parser.add_argument("--dataset-name", type=str, default=None,
                           help="Имя директории локального датасета внутри dataset-dir")
    ...
    def setup_local_dataset(self):
        """Настраивает пути для локального датасета."""
        if self.args.dataset_name:
            # Используем указанное имя поддиректории
            dataset_path = os.path.join(self.args.dataset_dir, self.args.dataset_name)
        else:
            # Используем саму директорию как датасет
        ...
        # Проверяем наличие необходимых поддиректорий
        required_subdirs = ['train/images', 'train/labels']
        for subdir in required_subdirs:
            if not os.path.exists(os.path.join(dataset_path, subdir)):
                print(f"Ошибка: в датасете отсутствует необходимая поддиректория: {subdir}")
                return False
        
        # Ищем или создаем data.yaml
        data_yaml_path = os.path.join(dataset_path, "data.yaml")
        if not os.path.exists(data_yaml_path):
            print(f"Предупреждение: data.yaml не найден в {dataset_path}, будет создан новый файл")
            
            # Ищем имена классов или используем аргументы командной строки
            class_names = self.args.class_names
            if not class_names:
                # Проверяем наличие классов по именам файлов классов в директории labels
                labels_dir = os.path.join(dataset_path, 'train', 'labels')
                class_names = self.detect_classes_from_labels(labels_dir)
                
                if not class_names:
                    print("Предупреждение: не удалось определить имена классов, используются значения по умолчанию")
                    class_names = ['class0', 'class1']
            
            # Создаем базовый data.yaml
            yaml_content = {
                'path': dataset_path,
            ...
            print(f"Создан новый data.yaml с классами: {class_names}")
        
        self.dataset_path = dataset_path
        self.data_yaml_path = data_yaml_path
        return True
    
    def detect_classes_from_labels(self, labels_dir):
        """Пытается определить классы из файлов аннотаций."""
        try:
            class_ids = set()
            label_files = [f for f in os.listdir(labels_dir) if f.endswith('.txt')]
            
            # Читаем первые 100 файлов для определения классов
            for i, label_file in enumerate(label_files[:100]):
            ...
            # Создаем имена классов на основе найденных ID
            if class_ids:
                return [f'class{id}' for id in sorted(class_ids)]
    ...
    def augment_dataset(self):
        ...
        if not self.dataset_path:
            print("Ошибка: путь к датасету не определен. Сначала загрузите датасет или укажите локальный.")
            return False
    ...
    def run(self):
        ...
        if self.args.mode in ['download', 'augment', 'full']:
            if self.args.data_source == 'roboflow' and self.args.mode in ['download', 'full']:
                print("\n=== Шаг 1: Загрузка датасета из Roboflow ===")
                success = self.download_from_roboflow()
            elif self.args.data_source == 'local':
                print("\n=== Шаг 1: Настройка локального датасета ===")
                success = self.setup_local_dataset()
            
            if not success:
                print("Ошибка настройки датасета. Дальнейшая обработка прекращена.")
                return
